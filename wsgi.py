...
from_ = "transportation@woxsen.edu.in"
pwd = "Koc78391"
...
def create_csv(dict_data,bus_route):
    with open(f"{bus_route}.csv", "w") as f:
...
def download_csv():
    ...
    if bus_id in ['1','1.1','1.2','1.3','1.4','1.5']:
        bus_route = 'Gachibowli'
    elif bus_id in ['2','2.1','2.2','2.3','2.4','2.5']:
        bus_route = 'Ameerpet'
    ...
    create_csv(dict_result,bus_route)
    
    # send file data.csv
    return send_file(f"./{bus_route}.csv", as_attachment=True)
...
def home():
    ...
    # Create time objects for the desired time range 8:30 to 10 AM on Friday and 2 to 4 PM on saturday
    start_time_f = dt_time(1, 30)
    end_time_f = dt_time(18, 0) #change back to 10

    start_time_s = dt_time(14,0)

    end_time_s = dt_time(16,0)
    
    # Check if it's Friday (weekday 4) or Saturday (weekday 5)
    if (datetime.today().weekday() == 4 and (start_time_f <= current_time <= end_time_f)) or \
    (datetime.today().weekday() == 5 and (start_time_s <= current_time <= end_time_s)):
        print("DA")
...
def search_bus():
    ...
    for bus in bus_result:
        ...
        res = cursor.fetchone()
        if res is None:
            add_bus_seats_query = f"INSERT INTO bus_seats (bus_number, total_seats, date,fare) VALUES ('{bus_number}', 38, '{bus_date}',{fare})"
...
def payment():
    ...
    additional_info = {
        ...
        "additional_info2": "Woxsen Domestic Transport Booking",
        "additional_info3": "transportation@woxsen.edu.in",
        "additional_info4": "NA",
        "additional_info5": "NA",
        "additional_info6": "NA",
        "additional_info7": "NA"
